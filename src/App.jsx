...
// ============ NOTES ============
function Notes(){const[notes,setNotes]=useState(()=>{try{const s=localStorage.getItem("mb_notes");return s?JSON.parse(s):[];}catch{return[];}});const[input,setInput]=useState("");const updateNotes=n=>{setNotes(n);localStorage.setItem("mb_notes",JSON.stringify(n))};const add=()=>{if(!input.trim())return;updateNotes([{text:input.trim(),time:new Date().toLocaleString(),id:Date.now()},...notes]);setInput("")};const rm=id=>updateNotes(notes.filter(n=>n.id!==id));return <section style={S.card}><h2 style={S.cardTitle}><span style={{color:"#f472b6"}}>◆</span> Quick Notes<Info text="Scratch pad for trade ideas, research questions, and reminders. Notes persist in browser localStorage across sessions." /></h2><div style={{display:"flex",gap:8,marginBottom:notes.length?10:0}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Trade idea, research note, reminder..." style={{flex:1,background:"#080c16",border:"1px solid #1e293b",borderRadius:8,padding:"8px 12px",color:"#e2e8f0",fontSize:12,fontFamily:"'Space Grotesk',sans-serif",outline:"none"}}/><button onClick={add} style={{...S.btn,padding:"8px 16px"}}>Add</button></div>{notes.length===0&&<p style={{color:"#64748b",fontSize:11,textAlign:"center",padding:"8px 0"}}>Jot down ideas, questions, or reminders</p>}{notes.map(n=><div key={n.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 10px",borderRadius:6,background:"#080c16",border:"1px solid #1e293b",marginBottom:4}}><div><div style={{color:"#e2e8f0",fontSize:12}}>{n.text}</div><div style={{color:"#64748b",fontSize:9,fontFamily:"'JetBrains Mono',monospace",marginTop:1}}>{n.time}</div></div><button onClick={()=>rm(n.id)} style={{background:"none",border:"none",color:"#64748b",cursor:"pointer",fontSize:14,padding:"4px 8px"}}>×</button></div>)}</section>;}

// ============ HERO + CMD ============
function Hero(){const[ph,setPh]=useState(0);useEffect(()=>{const timers=[setTimeout(()=>setPh(1),350),setTimeout(()=>setPh(2),1000),setTimeout(()=>setPh(3),1800),setTimeout(()=>setPh(4),2600)];return()=>timers.forEach(clearTimeout)},[]);return <div style={{position:"fixed",inset:0,background:"#030508",zIndex:9999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",transition:"opacity 1s ease",opacity:ph>=4?0:1,pointerEvents:ph>=4?"none":"all"}} onClick={()=>setPh(4)}><div style={{position:"absolute",width:600,height:600,borderRadius:"50%",background:"radial-gradient(circle,rgba(52,211,153,0.06) 0%,transparent 70%)",animation:"breathe 4s ease-in-out infinite"}}/><div style={{position:"absolute",width:400,height:400,borderRadius:"50%",background:"radial-gradient(circle,rgba(96,165,250,0.04) 0%,transparent 70%)",animation:"breathe 5s ease-in-out infinite",animationDelay:"1s",left:"35%",top:"55%"}}/><div style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg, transparent, rgba(52,211,153,0.3), transparent)",opacity:ph>=1?1:0,transition:"opacity 1s"}} /><div style={{position:"relative",textAlign:"center"}}><div style={{fontSize:12,fontFamily:"'JetBrains Mono',monospace",color:"#34d399",letterSpacing:8,textTransform:"uppercase",marginBottom:28,opacity:ph>=1?1:0,transform:`translateY(${ph>=1?0:10}px)`,transition:"all 0.8s cubic-bezier(0.4,0,0.2,1)"}}>$ terminal / active</div><h1 className="hero-name" style={{fontSize:72,fontWeight:700,fontFamily:"'Instrument Serif',serif",color:"#f1f5f9",lineHeight:1,marginBottom:18,opacity:ph>=2?1:0,transform:`translateY(${ph>=2?0:20}px) scale(${ph>=2?1:0.95})`,transition:"all 1s cubic-bezier(0.4,0,0.2,1)",textShadow:"0 4px 40px rgba(52,211,153,0.12), 0 0 80px rgba(52,211,153,0.04)"}}>Mason J. Bennett</h1><p className="hero-sub" style={{fontSize:15,color:"#94a3b8",letterSpacing:2,opacity:ph>=3?1:0,transform:`translateY(${ph>=3?0:10}px)`,transition:"all 0.8s cubic-bezier(0.4,0,0.2,1)",fontFamily:"'Space Grotesk',sans-serif"}}>M.S. Finance '26 · University of Arkansas · Investment Banking & Private Equity</p><div style={{display:"flex",gap:16,marginTop:32,justifyContent:"center",opacity:ph>=3?1:0,transform:`translateY(${ph>=3?0:10}px)`,transition:"all 0.8s cubic-bezier(0.4,0,0.2,1) 0.2s"}}>{["IB","PE","TA"].map(t=><span key={t} style={{fontSize:10,padding:"4px 14px",borderRadius:20,border:"1px solid rgba(52,211,153,0.2)",color:"#34d399",fontFamily:"'JetBrains Mono',monospace",letterSpacing:2,background:"rgba(52,211,153,0.05)"}}>{t}</span>)}</div></div><div style={{position:"absolute",bottom:36,fontSize:9,color:"#475569",fontFamily:"'JetBrains Mono',monospace",letterSpacing:3,textTransform:"uppercase",opacity:ph>=1&&ph<4?0.8:0,transition:"opacity 1.2s"}}>click anywhere to skip</div></div>;}
  ...
  return <div style={{ animation: "fadeUp 0.4s ease both" }}>
    ...
    <div style={{ ...S.card, marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={S.cardTitle}><span style={{ color: "#34d399" }}>◆</span> Investment Thesis Board<Info text="Structured investment write-ups with bull/bear stance, catalysts, and risks. Click any thesis to expand details. Add new theses with the + button." /></h2>
    ...
    <div style={S.card}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={S.cardTitle}><span style={{ color: "#a78bfa" }}>◆</span> Weekly Market Commentary<Info text="Personal market journal — macro views, sector analysis, and trade ideas. Entries are date-stamped and persist in browser storage." /></h2>
...
export default function App() {
  ...
  return <div style={S.root}>
    ...
    <div className="status-bar" style={{ background: "linear-gradient(90deg, #040609, #060a14, #040609)", borderBottom: "1px solid #162035", padding: "6px 32px", display: "flex", justifyContent: "space-between", fontSize: 9, fontFamily: "JetBrains Mono, monospace", color: "#64748b", position: "relative", zIndex: 2 }}>
      ...
      <span><span style={{ color: "#34d399" }}>●</span> Open to opportunities · IB / PE / Transaction Advisory</span>
    </div>
    ...
    <header style={S.header}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ fontSize: 9, padding: "4px 12px", borderRadius: 20, background: "linear-gradient(135deg, rgba(52,211,153,0.12), rgba(52,211,153,0.06))", color: "#34d399", border: "1px solid #34d39925", fontFamily: "JetBrains Mono, monospace", boxShadow: "0 0 12px rgba(52,211,153,0.08)" }}>M.S. Finance '26</div>
      </div>
    ...
    <div style={{ height: 2, background: "linear-gradient(90deg, transparent, #34d399, #60a5fa, transparent)", backgroundSize: "200% 100%", animation: "gradientShift 10s ease infinite", opacity: 0.5 }} />
    ...
    <main style={{ padding: 32, maxWidth: 1300, margin: "0 auto", position: "relative", zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(16px)", transition: "all 0.6s ease 0.3s" }}>
        ...
        <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <section style={{ ...S.card, animation: "fadeUp 0.5s ease 0.16s both" }}>
              <h2 style={S.cardTitle}><span style={{ color: "#60a5fa" }}>◆</span> Quick Links<Info text="One-click access to essential finance platforms — Bloomberg, Reuters, EDGAR, TradingView, and more." /></h2>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{QLINKS.map(s => <a key={s.n} href={s.u} target="_blank" rel="noopener noreferrer" style={{...S.chip, boxShadow: "0 2px 6px rgba(0,0,0,0.2)"}} onMouseEnter={e => { e.currentTarget.style.borderColor = "#34d39940"; e.currentTarget.style.color = "#34d399"; e.currentTarget.style.background = "rgba(52,211,153,0.06)"; e.currentTarget.style.boxShadow = "0 4px 12px rgba(52,211,153,0.08)"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "#1e293b"; e.currentTarget.style.color = "#94a3b8"; e.currentTarget.style.background = "rgba(8,12,22,0.8)"; e.currentTarget.style.boxShadow = "0 2px 6px rgba(0,0,0,0.2)"; e.currentTarget.style.transform = "none"; }}>{s.n}</a>)}</div>
            </section>
          </div>
        </div>
        <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
          <section style={{ ...S.card, animation: "fadeUp 0.5s ease 0.2s both" }}><h2 style={S.cardTitle}><span style={{ color: "#fbbf24" }}>◆</span> Sector Heatmap<Info text="Visual map of sector performance by GICS sector. Size reflects relative market cap weight. Green = positive, red = negative, yellow = flat. Click any ticker to open TradingView. Source: Finnhub.io real-time quotes (5-min cache)." link="https://www.investopedia.com/terms/s/sector-analysis.asp" linkLabel="Sector rotation & analysis" /></h2><HeatMap finnhubKey={finnhubKey} /></section>
      ...
      {tab === "projects" && <div style={{ animation: "fadeUp 0.4s ease both" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(360px,100%),1fr))", gap: 14 }}>{PROJECTS.map((p, i) => <div key={i} style={{ ...S.pCard, ...(hovP === i ? { borderColor: "#34d39950", transform: "translateY(-6px) scale(1.01)", boxShadow: "0 20px 50px rgba(52,211,153,0.12), 0 0 0 1px rgba(52,211,153,0.15), 0 0 40px rgba(52,211,153,0.05)" } : {}), animation: "fadeUp 0.5s ease both", animationDelay: `${i * 0.07}s` }} onMouseEnter={() => setHovP(i)} onMouseLeave={() => setHovP(null)}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><span style={{ fontSize: 9, color: "#64748b", fontFamily: "JetBrains Mono, monospace", letterSpacing: 1 }}>PROJECT_{String(i + 1).padStart(2, "0")}{p.completed && <span style={{ marginLeft: 8, color: "#475569" }}>· {p.completed}</span>}</span><div style={{ display: "flex", alignItems: "center", gap: 6 }}>{p.updated && <span style={{ fontSize: 8, color: "#475569", fontFamily: "JetBrains Mono, monospace" }}>Updated {p.updated}</span>}<span style={{ fontSize: 9, padding: "3px 10px", borderRadius: 20, background: p.status === "In Progress" ? "#fbbf2408" : "#34d39908", color: p.status === "In Progress" ? "#fbbf24" : "#34d399", fontFamily: "JetBrains Mono, monospace" }}>{p.status}</span></div></div>
      ...
      {tab === "about" && <div style={{ animation: "fadeUp 0.4s ease both", maxWidth: 820 }}>
        ...
        <div style={{ background: "linear-gradient(135deg, rgba(52,211,153,0.5), rgba(30,45,74,0.8), rgba(96,165,250,0.35))", borderRadius: 18, padding: 1, boxShadow: "0 12px 48px rgba(52,211,153,0.08)" }}>
        <div style={{ ...S.card, background: "linear-gradient(135deg,#080c16,#0d1828,#0b1120)", border: "none", borderRadius: 16, position: "relative", overflow: "hidden" }}>
          ...
          <div className="bio-layout" style={{ display: "flex", gap: 28, alignItems: "flex-start", flexWrap: "wrap", position: "relative" }}>
            <div style={{ flexShrink: 0, borderRadius: 22, padding: 2, background: "linear-gradient(135deg, #34d399, #60a5fa)", boxShadow: "0 12px 40px rgba(52,211,153,0.15)" }}>
        ...
        <div style={{ ...S.card, marginBottom: 14 }}>
          ...
          <div style={{ position: "relative", paddingLeft: 22 }}><div style={{ position: "absolute", left: 5, top: 5, bottom: 5, width: 2, background: "linear-gradient(180deg,#34d399,#60a5fa,#1e293b)", borderRadius: 1 }} />
        ...
        <div style={{ ...S.card, marginBottom: 14 }}>
          ...
          <div style={{ marginBottom: 14 }}><div style={{ fontSize: 8, color: "#64748b", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8, fontFamily: "JetBrains Mono, monospace" }}>Core Finance</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{["Financial Modeling", "Valuation (DCF, LBO, Comps)", "Investment Analysis", "Transaction Analysis", "Portfolio Management", "Econometrics", "Regression Analysis", "Quantitative Research"].map(s => <span key={s} style={{ fontSize: 11, padding: "6px 14px", borderRadius: 8, background: "linear-gradient(135deg, rgba(52,211,153,0.08), rgba(52,211,153,0.04))", color: "#34d399", border: "1px solid rgba(52,211,153,0.15)", transition: "all 0.25s", cursor: "default", boxShadow: "0 2px 6px rgba(52,211,153,0.04)" }} onMouseEnter={e=>{e.currentTarget.style.background="rgba(52,211,153,0.12)";e.currentTarget.style.boxShadow="0 4px 12px rgba(52,211,153,0.1)";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.background="linear-gradient(135deg, rgba(52,211,153,0.08), rgba(52,211,153,0.04))";e.currentTarget.style.boxShadow="0 2px 6px rgba(52,211,153,0.04)";e.currentTarget.style.transform="none"}}>{s}</span>)}</div></div>
          <div><div style={{ fontSize: 8, color: "#64748b", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8, fontFamily: "JetBrains Mono, monospace" }}>Tools</div><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{["Excel", "Python", "Stata", "RStudio", "SQL", "PowerPoint", "Streamlit"].map(s => <span key={s} style={{ fontSize: 11, padding: "6px 14px", borderRadius: 8, background: "linear-gradient(145deg, #0c1222, #080c16)", color: "#94a3b8", border: "1px solid #1e2d4a", transition: "all 0.25s", cursor: "default", boxShadow: "0 2px 6px rgba(0,0,0,0.2)" }} onMouseEnter={e=>{e.currentTarget.style.borderColor="#60a5fa40";e.currentTarget.style.color="#60a5fa";e.currentTarget.style.boxShadow="0 4px 12px rgba(96,165,250,0.08)";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1e2d4a";e.currentTarget.style.color="#94a3b8";e.currentTarget.style.boxShadow="0 2px 6px rgba(0,0,0,0.2)";e.currentTarget.style.transform="none"}}>{s}</span>)}</div></div>
        </div>
        <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
          <div style={S.card}>
            <h2 style={S.cardTitle}><span style={{ color: "#fbbf24" }}>◆</span> Reading List</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{READING.map((b, i) => <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: "linear-gradient(145deg, #0c1222, #080c16)", border: "1px solid #1e2d4a", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "all 0.25s" }} onMouseEnter={e=>{e.currentTarget.style.borderColor="#fbbf2430";e.currentTarget.style.transform="translateX(4px)"}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1e2d4a";e.currentTarget.style.transform="none"}}><div><div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 500 }}>{b.title}</div><div style={{ color: "#64748b", fontSize: 10 }}>{b.author}</div></div><span style={{ fontSize: 8, padding: "3px 10px", borderRadius: 10, fontFamily: "JetBrains Mono, monospace", background: b.s === "Reading" ? "rgba(96,165,250,0.1)" : b.s === "Done" ? "rgba(52,211,153,0.1)" : "rgba(148,163,184,0.08)", color: b.s === "Reading" ? "#60a5fa" : b.s === "Done" ? "#34d399" : "#94a3b8", border: `1px solid ${b.s === "Reading" ? "rgba(96,165,250,0.2)" : b.s === "Done" ? "rgba(52,211,153,0.2)" : "rgba(148,163,184,0.15)"}` }}>{b.s === "Done" ? "Completed" : b.s === "Ref" ? "Reference" : b.s}</span></div>)}</div>
          </div>
          <div style={S.card}>
    ...
    <footer style={{ padding: "26px 32px", borderTop: "1px solid #162035", background: "linear-gradient(180deg, transparent, rgba(4,6,9,0.5))", boxShadow: "0 -4px 30px rgba(0,0,0,0.3)", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
      <div style={{ display: "flex", gap: 18, flexWrap: "wrap", justifyContent: "center" }}>
        {LINKS.map(l => <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer" style={{ color: "#64748b", fontSize: 10, fontFamily: "'JetBrains Mono',monospace", textDecoration: "none", letterSpacing: 1, transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#34d399"} onMouseLeave={e => e.currentTarget.style.color = "#64748b"}>{l.label}</a>)}
      </div>
      <div style={{ color: "#475569", fontSize: 9, fontFamily: "JetBrains Mono, monospace", display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
    ...
    <style>{`
      ...
      @keyframes borderGlow{0%{border-color:rgba(52,211,153,0.2)}50%{border-color:rgba(52,211,153,0.4)}100%{border-color:rgba(52,211,153,0.2)}}
...
const S = {
  ...
  tabA: { color: "#fff", background: "linear-gradient(135deg, rgba(52,211,153,0.15), rgba(96,165,250,0.1))", boxShadow: "0 0 20px rgba(52,211,153,0.15), 0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.08), inset 0 -2px 0 #34d399", fontWeight: 600 },
  card: { background: "linear-gradient(145deg, #0c1222, #0a0f1c)", border: "1px solid #1e2d4a", borderRadius: 16, padding: 24, boxShadow: "0 8px 32px rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.05) inset", transition: "all 0.3s cubic-bezier(0.4,0,0.2,1)" },
  ...
  pageTitle: { fontSize: 42, fontWeight: 700, fontFamily: "'Instrument Serif',serif", marginBottom: 10, letterSpacing: "-0.03em", lineHeight: 1.1, background: "linear-gradient(135deg, #f1f5f9, #34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", filter: "drop-shadow(0 4px 20px rgba(52,211,153,0.1))" },
  ...
  pCard: { display: "flex", flexDirection: "column", background: "linear-gradient(145deg, #0c1222, #0a0f1c)", border: "1px solid #1e2d4a", borderRadius: 16, padding: 24, transition: "all 0.4s cubic-bezier(0.4,0,0.2,1)", cursor: "pointer", boxShadow: "0 8px 32px rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.05) inset, inset 3px 0 0 #34d39930" },
