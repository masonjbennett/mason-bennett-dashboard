...
const PROJECTS = [
  { title: "Filings Terminal", desc: "A financial-data terminal that pulls eight years of reported financials for any of 10,387 SEC filers straight from EDGAR's XBRL API. No AI anywhere in the data path — every figure is the value the company filed, and every reported number on the page is a link that opens the exact filing it came from, so a sceptical reader can check any of it in one click. 276 line items across three tabs (Statements · Ratios · Valuation), with industry overlays that reshape the sheet by SIC code: banks get net interest income and an efficiency ratio, P&C insurers a combined ratio and Buffett's float, life insurers book value ex-AOCI, REITs a reconstructed NAREIT FFO, and broker-dealers the compensation ratio. The extraction engine is the real work — ordered tag fallbacks that span taxonomy eras, latest-filed-wins handling so restatements win over stale comparatives, and five distinct kinds of blank so a missing number tells you whether it was never reported, never tagged, or does not exist for that filer type. Verified against 50+ filers with a regression suite that drives the shipping serverless function.", tags: ["React", "SEC EDGAR XBRL", "Serverless", "Financial Reporting"], status: "Live", completed: "Aug 2026", updated: "Aug 12, 2026", url: "https://github.com/masonjbennett/filings-terminal", demo: "https://filings-terminal.vercel.app" },
...
const FEATURED = [
  { title: "Filings Terminal", note: "Live tool", img: "/projects/filings-terminal.png", blurb: "Eight years of reported financials for any of 10,387 SEC filers, pulled straight from EDGAR — no AI in the data path, and every figure links to the filing it came from.", tab: "projects" },
...
const ARTIFACTS = [
  ...
  { label: "Filings Terminal — SEC filings to a model-ready sheet, live tool", href: "https://filings-terminal.vercel.app", recruiterSafe: true },
...
const ClaimRow = ({ c }) => <div style={{display:"flex",gap:8,padding:"5px 8px",borderRadius:6,background:"#f6eee180"}}>
  <span style={{color:SC[c.status],fontSize:12,fontWeight:700,width:16,textAlign:"center",fontFamily:"'JetBrains Mono',monospace"}}>{SI[c.status]}</span>
  <div style={{flex:1}}>
    <div style={{fontSize:11,color:"#33302c",lineHeight:1.4}}>{c.claim}</div>
    {c.note&&<div style={{fontSize:10,color:"#6f675c",lineHeight:1.45}}>{c.note}</div>}
    {c.source&&<div style={{fontSize:9,color:"#a2977f",fontFamily:"'JetBrains Mono',monospace",marginTop:2}}>{c.source}</div>}
  </div>
...
{verify&&<div style={{padding:"10px 14px",borderRadius:10,background:"#f6eee1",border:"1px solid #e9ddc9",marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#8a8072",textTransform:"uppercase",letterSpacing:1.5}}>Verification</span>{(()=>{const bad=(verify.summary.unverified||0)+(verify.summary.discrepancy||0);return bad>0
  ...
  :<span style={{fontSize:11,padding:"2px 10px",borderRadius:20,fontFamily:"'JetBrains Mono',monospace",fontWeight:700,background:"#0d6d5610",color:"#0d6d56"}}>{verify.summary.total} sourced</span>;})()}
<span style={{fontSize:9,color:"#a2977f",fontFamily:"'JetBrains Mono',monospace"}}>{verify.summary.confidence_pct}% self-scored</span></div><button onClick={()=>setShowCl(!showCl)} style={{background:"none",border:"1px solid #e9ddc9",borderRadius:6,color:"#8a8072",fontSize:10,padding:"3px 8px",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace"}}>{showCl?"Hide":"Show"} {verify.summary.verified||0} sourced</button></div><div style={{width:"100%",height:3,borderRadius:2,background:"#e9ddc9",overflow:"hidden",display:"flex"}}><div style={{width:`${(verify.summary.verified/verify.summary.total)*100}%`,background:"#0d6d56"}}/><div style={{width:`${(verify.summary.discrepancy/verify.summary.total)*100}%`,background:"#b0741e"}}/><div style={{width:`${(verify.summary.unverified/verify.summary.total)*100}%`,background:"#b2342b"}}/></div>{verify.claims&&(()=>{const flagged=verify.claims.filter(c=>c.status!=="verified"),ok=verify.claims.filter(c=>c.status==="verified");return <>
  {flagged.length>0&&<div style={{display:"flex",flexDirection:"column",gap:3,marginTop:8}}>{flagged.map((c,i)=><ClaimRow key={`f${i}`} c={c}/>)}</div>}
  {flagged.length===0&&<div style={{fontSize:10,color:"#0d6d56",fontFamily:"'JetBrains Mono',monospace",marginTop:8,letterSpacing:0.5}}>Every extracted claim found a source.</div>}
  {showCl&&ok.length>0&&<div style={{display:"flex",flexDirection:"column",gap:3,marginTop:6}}>{ok.map((c,i)=><ClaimRow key={`v${i}`} c={c}/>)}</div>}
</>;})()}</div>}
    ...
    <main style={{ padding: 32, maxWidth: 1300, margin: "0 auto", position: "relative", zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(16px)", transition: "all 0.6s ease 0.3s" }}>
        ...
        <div id="model-rack" style={{ ...S.card, marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(210px,100%),1fr))", gap: 12 }}>
            {[
              ...
              { f: "dcf-template.xlsx", t: "Discounted Cash Flow", d: "Five years of unlevered free cash flow discounted at WACC, with a perpetuity-growth terminal value." },
              ...
              { f: "precedent-transactions-template.xlsx", t: "Precedent Transactions", d: "Acquisition multiples from past deals, applied to value a target at a control premium." },
              { f: "three-statement-template.xlsx", t: "Three-Statement Model", d: "A five-year income statement, cash flow, and balance sheet that ties out every year." },
        ...
        <Reveal><div style={{ padding: "8px 0 22px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 20 }}>
            {["Interviewing for analyst roles across wealth management, corporate finance, investment banking, and private equity",
              ...
              "Sitting the FINRA SIE in August 2026 — CFA Level I planned next",
      ...
      {tab === "recruiter" && <div style={{ animation: "fadeUp 0.4s ease both", maxWidth: 600, margin: "0 auto", textAlign: "center", padding: "14px 0 30px" }}>
        ...
        <div style={{ borderTop: "1px solid #e9ddc9", borderBottom: "1px solid #e9ddc9", padding: "14px 0", marginBottom: 20, display: "flex", flexDirection: "column", gap: 8 }}>
          {["4.0 GPA — M.S. Finance, Walton College of Business (May 2026)", "Info Tech Sector Analyst — Shollmier Investment Fund ($700K+, Spring 2026)", "Bloomberg Market Concepts certified · FINRA SIE sitting August 2026"].map(s => <div key={s} style={{ fontSize: 13, color: "#33302c", fontWeight: 500 }}>{s}</div>)}
        </div>
