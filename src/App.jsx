...
function Info({ text, link, linkLabel }) {
  ...
  return <span style={{ position: "relative", display: "inline-flex", marginLeft: 6 }} onMouseEnter={enter} onMouseLeave={leave}>
    ...
    {show && <div onMouseEnter={enter} onMouseLeave={leave} style={{ position: "absolute", bottom: "calc(100% + 4px)", left: "50%", transform: "translateX(-50%)", background: "#0f172a", border: "1px solid #1e2d4a", borderRadius: 10, padding: "12px 16px", fontSize: 11, color: "#cbd5e1", lineHeight: 1.6, width: 280, zIndex: 50, boxShadow: "0 8px 24px rgba(0,0,0,0.5), 0 0 0 1px rgba(52,211,153,0.05)", fontFamily: "'Space Grotesk',sans-serif", textTransform: "none", letterSpacing: 0, fontWeight: 400, animation: "fadeUp 0.15s ease", cursor: "default" }}>{text}{link && <a href={link} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 10, fontSize: 10, color: "#34d399", textDecoration: "none", fontFamily: "'JetBrains Mono',monospace", padding: "4px 10px", borderRadius: 6, background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.15)", transition: "all 0.2s" }} onMouseEnter={e=>{e.currentTarget.style.background="rgba(52,211,153,0.12)";e.currentTarget.style.borderColor="rgba(52,211,153,0.3)"}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(52,211,153,0.06)";e.currentTarget.style.borderColor="rgba(52,211,153,0.15)"}}>{linkLabel || "Learn more"} ↗</a>}</div>}
  </span>;
}
...
function HeatMap({ finnhubKey }){
  ...
  const gc=c=>{const v=parseFloat(c);return v>3?"#22c55e":v>1?"#4ade80":v>-1?"#fbbf24":v>-3?"#f87171":"#ef4444"};
...
export default function App() {
  ...
  useEffect(() => { const t = setTimeout(() => { setShowHero(false); setMounted(true); }, 4200); return () => clearTimeout(t); }, []);
  ...
  return <div style={S.root}>
    ...
    <main style={{ padding: 32, maxWidth: 1300, margin: "0 auto", position: "relative", zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(16px)", transition: "all 0.6s ease 0.3s" }}>
      ...
      {tab === "dashboard" && <div>
        ...
        <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
          <section style={{ ...S.card, animation: "fadeUp 0.5s ease 0.2s both", borderTop: "2px solid #fbbf2440" }}><h2 style={S.cardTitle}><span style={{ color: "#fbbf24" }}>◆</span> Sector Heatmap<Info text="Visual map of sector performance by GICS sector. Size reflects relative market cap weight. Green = positive, red = negative, yellow = flat. Click any ticker to open TradingView. Source: Finnhub.io real-time quotes (5-min cache)." link="https://www.investopedia.com/terms/s/sector-analysis.asp" linkLabel="Sector rotation & analysis" /></h2><HeatMap finnhubKey={finnhubKey} /></section>
      ...
      {tab === "about" && <div style={{ animation: "fadeUp 0.4s ease both", maxWidth: 820 }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #34d399, #60a5fa, #a78bfa)", borderRadius: 18, padding: 2, boxShadow: "0 12px 48px rgba(52,211,153,0.15), 0 0 60px rgba(52,211,153,0.05)", backgroundSize: "200% 200%", animation: "gradientShift 6s ease infinite" }}>
    ...
    <footer style={{ padding: "24px 32px", borderTop: "1px solid #162035", color: "#64748b", fontSize: 9, fontFamily: "JetBrains Mono, monospace", display: "flex", justifyContent: "center", gap: 10, background: "linear-gradient(180deg, transparent, rgba(4,6,9,0.5))", boxShadow: "0 -4px 30px rgba(0,0,0,0.3)" }}>
      <span>Mason J. Bennett</span><span>·</span><span>Walton College</span><span>·</span><span>© {new Date().getFullYear()}</span><span>·</span><span>⌘K or 1-6</span>
    </footer>
