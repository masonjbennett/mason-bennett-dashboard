...
const LINKS = [
  ...
  { label: "GitHub", url: "#", ic: "gh" },
  { label: "Resume", url: "#", ic: "cv" },
  { label: "Email", url: "mailto:", ic: "✉" },
];
const NEWS_CATS = [
...
async function fetchNews(cat) {
  try { const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 3000, tools: [{ type: "web_search_20250305", name: "web_search" }], messages: [{ role: "user", content: `Search for the latest ${cat.q}.\n${SRC_GUIDE}\nReturn ONLY a JSON array of top ${cat.count} articles: [{"title":"...","source":"...","summary":"one sentence","url":"...","time":"relative"}]. Raw JSON only.` }] }) }); const d = await r.json(); return JSON.parse(d.content.filter(b => b.type === "text").map(b => b.text).join("").replace(/```json|```/g, "").trim()); } catch { return null; }
}
async function fetchBriefing(type) {
  ...
  try { const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 4000, tools: [{ type: "web_search_20250305", name: "web_search" }], messages: [{ role: "user", content: p[type] }] }) }); const d = await r.json(); const raw = d.content.filter(b => b.type === "text").map(b => b.text).join("\n\n"); let text = raw, sources = []; const sep = raw.indexOf("---SOURCES---"); if (sep !== -1) { text = raw.slice(0, sep).trim(); try { sources = JSON.parse(raw.slice(sep + 13).trim().replace(/```json|```/g, "").trim()); } catch {} } if (!sources.length) { const m = text.match(/\[([A-Z][A-Za-z\s\.&']+?)\]/g); if (m) sources = [...new Set(m.map(x => x.slice(1, -1).trim()))].map(n => ({ name: n, url: SRC_URLS[n] || "#" })); } return { text, sources }; } catch { return null; }
}
async function verifyBriefing(t) {
  try { const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 3000, tools: [{ type: "web_search_20250305", name: "web_search" }], messages: [{ role: "user", content: `Fact-check this briefing. Extract factual claims, verify each via web search. Return ONLY JSON: {"summary":{"verified":0,"unverified":0,"discrepancy":0,"total":0,"confidence_pct":0},"claims":[{"claim":"...","status":"verified|unverified|minor_discrepancy","note":"...","source":"..."}]}\n\n"""\n${t}\n"""` }] }) }); const d = await r.json(); return JSON.parse(d.content.filter(b => b.type === "text").map(b => b.text).join("").replace(/```json|```/g, "").trim()); } catch { return null; }
}
async function fetchSoWhat(t, type) {
...
// ============ HOOKS ============
function usePrices(tickers) { const [p, setP] = useState(() => tickers.map(t => ({ ...t, price: (80 + Math.random() * 450).toFixed(2), change: (Math.random() * 7 - 3.5).toFixed(2) }))); useEffect(() => { const iv = setInterval(() => setP(prev => prev.map(t => { const d = (Math.random() - 0.47) * 1.8; return { ...t, price: Math.max(1, parseFloat(t.price) + d).toFixed(2), change: (parseFloat(t.change) + d * 0.08).toFixed(2) }; })), 3000); return () => clearInterval(iv); }, []); return p; }
...
function RegimeIndicator() {
  ...
  const load = async () => { setLoading(true); setError(false); const r = await fetchRegime(); if (r) setData(r); else setError(true); setLoading(false); };
...
// ============ EARNINGS CALENDAR ============
function EarningsCal() {
  const [data, setData] = useState(null), [loading, setLoading] = useState(false), [error, setError] = useState(false);
  const load = async () => { setLoading(true); setError(false); const r = await fetchEarnings(); if (r) setData(r); else setError(true); setLoading(false); };
...
// ============ NEWS FEED ============
function NewsFeed(){const[articles,setArticles]=useState({}),[loading,setLoading]=useState({}),[activeCat,setActiveCat]=useState("markets"),[lastFetch,setLastFetch]=useState({});const[auto,setAuto]=useState(true),[cd,setCd]=useState(()=>getSmartInterval().interval),[fetchingAll,setFetchingAll]=useState(false),[si,setSi]=useState(()=>getSmartInterval());useEffect(()=>{const iv=setInterval(()=>setSi(getSmartInterval()),60000);return()=>clearInterval(iv)},[]);const fetchCat=useCallback(async cat=>{setLoading(p=>({...p,[cat.id]:true}));const r=await fetchNews(cat);if(r){setArticles(p=>({...p,[cat.id]:r}));setLastFetch(p=>({...p,[cat.id]:new Date()}))}setLoading(p=>({...p,[cat.id]:false}))},[]);const fetchAll=useCallback(async()=>{setFetchingAll(true);for(const c of NEWS_CATS)await fetchCat(c);setFetchingAll(false);setCd(getSmartInterval().interval)},[fetchCat]);useEffect(()=>{if(!auto)return;const iv=setInterval(()=>{setSi(getSmartInterval());setCd(c=>{if(c<=1){fetchAll();return getSmartInterval().interval}return c-1})},60000);return()=>clearInterval(iv)},[auto,fetchAll]);const cat=NEWS_CATS.find(c=>c.id===activeCat),arts=articles[activeCat],total=Object.values(articles).reduce((s,a)=>s+(a?a.length:0),0);
return <div><Briefings/><div style={{height:16}}/>
...
function ResearchTab() {
  const [theses, setTheses] = useState(INIT_THESES);
  const [commentary, setCommentary] = useState(INIT_COMMENTARY);
...
export default function App() {
  ...
  return <div style={S.root}>
    ...
    <div style={{ position: "fixed", inset: 0, opacity: 0.012, backgroundImage: "url('data:image/svg+xml,%3Csvg viewBox=%220 0 256 256%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22n%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.9%22 numOctaves=%224%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23n)%22/%3E%3C/svg%3E')", pointerEvents: "none" }} />

    <div style={{ background: "#050810", borderBottom: "1px solid #0b1120", padding: "5px 32px", display: "flex", justifyContent: "space-between", fontSize: 9, fontFamily: "JetBrains Mono, monospace", color: "#64748b", position: "relative", zIndex: 2 }}>
    ...
    <div style={{ overflow: "hidden", borderBottom: "1px solid #0b1120", background: "rgba(5,8,16,0.6)", padding: "7px 0" }}><div style={{ display: "flex", gap: 36, animation: "scroll 55s linear infinite", width: "max-content", fontFamily: "JetBrains Mono, monospace", fontSize: 11 }}>{[...prices, ...prices].map((t, i) => <span key={i} style={{ display: "flex", gap: 8, whiteSpace: "nowrap" }}><span style={{ color: "#64748b", fontWeight: 600 }}>{t.symbol}</span><span style={{ color: "#94a3b8" }}>${t.price}</span><span style={{ color: parseFloat(t.change) >= 0 ? "#34d399" : "#f87171", fontWeight: 600 }}>{parseFloat(t.change) >= 0 ? "+" : ""}{t.change}%</span></span>)}</div></div>
    ...
    <main style={{ padding: 32, maxWidth: 1300, margin: "0 auto", position: "relative", zIndex: 1, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(16px)", transition: "all 0.6s ease 0.3s" }}>
      ...
      {tab === "news" && <div style={{ animation: "fadeUp 0.4s ease both" }}><NewsFeed /></div>}
      ...
      {tab === "projects" && <div style={{ animation: "fadeUp 0.4s ease both" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(360px,1fr))", gap: 14 }}>{PROJECTS.map((p, i) => <div key={i} style={{ ...S.pCard, ...(hovP === i ? { borderColor: "#34d39940", transform: "translateY(-3px)", boxShadow: "0 14px 40px rgba(52,211,153,0.06)" } : {}), animation: "fadeUp 0.5s ease both", animationDelay: `${i * 0.07}s` }} onMouseEnter={() => setHovP(i)} onMouseLeave={() => setHovP(null)}>
      ...
      {tab === "about" && <div style={{ animation: "fadeUp 0.4s ease both", maxWidth: 820 }}>
        ...
        <div style={{ ...S.card, margin: "14px 0", display: "flex", justifyContent: "space-around", padding: "20px 32px" }}>
          {[["Master's GPA", "4.0", "Walton College"], ["Undergrad GPA", "3.62", "Dean's List ×4"], ["Projects", "5", "Academic & Independent"], ["Graduation", "May 2026", "M.S. Finance"]].map(([label, val, sub], i) => (
            <div key={label} style={{ textAlign: "center", position: "relative" }}>
              ...
              {i < 3 && <div style={{ position: "absolute", right: -40, top: "15%", height: "70%", width: 1, background: "#1e293b" }} />}
            </div>
          ))}
        </div>
    ...
    <footer style={{ padding: "18px 32px", borderTop: "1px solid #0b1120", color: "#64748b", fontSize: 9, fontFamily: "JetBrains Mono, monospace", display: "flex", justifyContent: "center", gap: 10 }}>
...
const S = {
  ...
  header: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 32px", borderBottom: "1px solid #0b1120", background: "rgba(6,10,20,0.88)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 100 },
  tab: { background: "none", border: "none", color: "#64748b", fontSize: 12, padding: "7px 12px", cursor: "pointer", borderRadius: 8, fontWeight: 500, transition: "all 0.2s", display: "flex", alignItems: "center" },
  tabA: { color: "#e2e8f0", background: "#1e293b40" },
  card: { background: "#0b1120", border: "1px solid #1e293b", borderRadius: 14, padding: 22 },
  cardTitle: { fontSize: 10, fontWeight: 600, color: "#94a3b8", marginBottom: 14, fontFamily: "'JetBrains Mono',monospace", textTransform: "uppercase", letterSpacing: 2 },
  pageTitle: { fontSize: 30, fontWeight: 700, fontFamily: "'Instrument Serif',serif", color: "#e2e8f0", marginBottom: 8 },
  ...
  pCard: { display: "flex", flexDirection: "column", background: "#0b1120", border: "1px solid #1e293b", borderRadius: 14, padding: 22, transition: "all 0.3s ease", cursor: "pointer" },
  ...
  input: { background: "#0b1120", border: "1px solid #1e293b", borderRadius: 8, padding: "8px 12px", color: "#e2e8f0", fontSize: 12, fontFamily: "'Space Grotesk',sans-serif", outline: "none", width: "100%" },
};
